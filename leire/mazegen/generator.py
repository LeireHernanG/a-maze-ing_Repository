...
class MazeGenerator:
    ...
    def __init__(
                    ...
                    exit: tuple[int, int], perfect: bool, seed: int = 42
                ):
        self.width = width
        self.height = height
        self.entry = entry
    ...
    def __accessible_neighbors(
        ...
        neighbors = []
        points = ['N', 'E', 'S', 'W']
        cell_value = self.structure[row, col]
        for num in range(4):
    ...
    def __compute_solution(self) -> None:
        """Compute the shortest solutions from entry to exit."""
        queue: deque[
            tuple[
                tuple[int, int],
                list[str],
    ...
    def __remove_wall(self, neighbor: tuple[int, int, int]) -> tuple[int, int]:
        ...
        self.structure[row, col] &= ~(1 << wall)
        self.structure[nrow, ncol] &= ~(1 << opposite)
        return (ncol, nrow)
                    ...
                    for sum_col in [1, 0, -1]:
                        for sum_row in [1, 0, -1]:
                            ...
                            if (0 <= ncol < self.width and
                               0 <= nrow < self.height):
                                if (self.structure[nrow][ncol] == 0 or
                                   self.structure[nrow][ncol] == 15):
    ...
    def __make_no_perfect(self) -> None:
        ...
        entry_walls = self.__possible_walls(self.entry)
        if entry_walls:
            self.__remove_wall(
                (self.entry[0], self.entry[1], random.choice(entry_walls)))
